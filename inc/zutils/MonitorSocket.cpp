...
#include "EventTypes.h" // for MODULE_STRING
ZLOG_MODULE_INIT(MODULE_STRING);
...
MonitorSocketNotification::GetId() const
{
  return(this->_id);
}

bool
MonitorSocketNotification::GetId(MonitorSocketNotification::ID id_)
{
  this->_id = id_;
  return(true);
}
...
MonitorSocket::ObserveEvent(SHARED_PTR(zSocket::Notification) notification_)
{
  ...
  case zSocket::Notification::ID_PKT_RCVD:
  {
    ...
    n->GetId(MonitorSocketNotification::ID_PKT_RCVD);
