...
bool Handler::Register(Observer *obs_)
{
    if (!obs_)
    {
        return (false);
...
bool Handler::Bind(Socket *sock_)
{
    bool status = false;
    if (!sock_ || this->_sock)
    {
...
bool Handler::Connect(Socket *sock_)
{
    bool status = false;
    if (!sock_ || this->_sock)
    {
...
bool Handler::Listen(Socket *sock_)
{
    bool status = false;
    if (!sock_)
    {
...
void Handler::Close(Socket *sock_)
{
...
ssize_t Handler::Send(const zSocket::Address *to_, zSocket::Buffer *sb_)
{
    ssize_t size = -1;
    if (this->_sock)
    {
...
ssize_t Handler::Send(const zSocket::Address *to_, const std::string &str_)
{
    ssize_t size = -1;
    if (this->_sock)
    {
...
ssize_t Handler::Broadcast(zSocket::Buffer *sb_)
{
    ssize_t size = -1;
    if (this->_sock)
    {
...
ssize_t Handler::Broadcast(const std::string &str_)
{
    ssize_t size = -1;
    if (this->_sock)
    {
...
void Handler::_notify(zSocket::Socket *sock_, zSocket::Address *addr_, zSocket::Buffer *buf_)
{
    bool status = false;
