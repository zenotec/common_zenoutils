...
static std::string _get_ipaddr_string( struct sockaddr_in *sockaddr_ )
{
...
static std::string _get_port_string( struct sockaddr_in *sockaddr_ )
{
    char port[10] = { 0 };
...
static bool _get_sockaddr( const InetAddress *addr_, struct sockaddr_in *sockaddr_ )
{
...
static bool _get_inetaddress( struct sockaddr_in *sockaddr_, InetAddress *addr_ )
{
...
InetAddress::InetAddress() :
        zSocket::Address( zSocket::Address::TYPE_INET )
{
...
bool InetAddress::operator ==( const InetAddress &other_ ) const
        {
    bool status = true;
...
bool InetAddress::SetAddress( const std::string &addr_ )
{
    bool status = false;
    char *str = 0;
    char *c = 0;
...
std::string InetAddress::GetIpAddr() const
{
    return (this->_ipaddr);
}

std::string InetAddress::GetPort() const
{
    return (this->_port);
}

std::string InetAddress::GetNetmask() const
{
    return (this->_netmask);
}

std::string InetAddress::GetBroadcast() const
{
    return (this->_bcast);
}

bool InetAddress::SetIpAddr( const std::string &ipaddr_ )
{
    bool status = true;
...
bool InetAddress::SetPort( const std::string &port_ )
{
    bool status = false;
    int port = 0;
...
bool InetAddress::SetNetmask( const std::string &netmask_ )
{
    bool status = true;
    // TODO: Add error checking
...
bool InetAddress::SetBroadcast( const std::string &bcast_ )
{
    bool status = true;
    // TODO: Add error checking
    this->_bcast = bcast_;
...
bool InetSocket::_open()
{
    ...
    if (!this->_sock)
    {
        ...
        {
            ZLOG_CRIT(
                    "InetSocket::_open: Cannot configure socket: " + std::string(strerror(errno)) );
            return (false);
        }
...
bool InetSocket::_bind()
{
    ...
    {
        ZLOG_CRIT( "InetSocket::_bind: Cannot convert socket address: "
                + std::string(strerror(errno)) );
        return (false);
    }
...
bool InetSocket::_connect()
{
    ...
    if (!_get_sockaddr( &this->_inetaddr, &addr ))
    {
        ZLOG_CRIT( "InetSocket::_connect: Cannot convert socket address: "
                + std::string(strerror(errno)) );
        return (false);
    }
...
ssize_t InetSocket::_send( const zSocket::Address *addr_, zSocket::Buffer *sb_ )
{
    ...
    {
        ZLOG_CRIT( std::string( "InetSocket::_send: Invalid address type: " +
    ...
    else
    {
        ...
        {
            ZLOG_CRIT( "InetSocket::_send: Cannot convert socket address: " +
                    std::string(strerror(errno)) );
        }
        ...
        if (bytes_sent < 0)
        {
            ZLOG_ERR( std::string("InetSocket::_send: Cannot send packet: " +
                    std::string(strerror(errno))) );
        }
...
ssize_t InetSocket::_broadcast( zSocket::Buffer *sb_ )
{
...
ssize_t InetSocket::_recv( zSocket::Address *addr_, zSocket::Buffer *sb_ )
{
    if (!this->_sock)
    {
