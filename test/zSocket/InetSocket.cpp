...
zSocketTest_InetSocketSendReceiveLoop(void* arg_)
{
  ...
  // Create new socket address and validate
  zSocket::InetAddress srcAddr("lo");
  TEST_EQ(std::string("127.0.0.1:0"), srcAddr.GetAddress());

  // Create new socket address and validate
  zSocket::InetAddress dstAddr;
  TEST_EQ(std::string("0.0.0.0:0"), dstAddr.GetAddress());

  // Set socket address
  TEST_TRUE(srcAddr.SetPort(std::string("9877")));
...
zSocketTest_InetSocketSendReceiveSock2Sock(void* arg_)
{
  ...
  // Set socket address
  TEST_TRUE(srcAddr.SetAddress(std::string("127.0.0.1:9878")));
  TEST_EQ(std::string("127.0.0.1:9878"), srcAddr.GetAddress());

  // Set socket address
  TEST_TRUE(dstAddr.SetAddress(std::string("127.0.0.2:9878")));
...
zSocketTest_InetSocketObserver(void* arg_)
{
  ...
  // Set socket address
  TEST_TRUE(srcAddr.SetAddress(std::string("127.0.0.1:9879")));
