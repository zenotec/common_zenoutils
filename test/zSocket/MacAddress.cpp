...
zSocketTest_MacAddressGetSet(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# zSocketTest_MacAddressGetSet()");
  ZLOG_DEBUG("#############################################################");

  // Create new socket address and validate
  MacAddress myAddr;
  TEST_EQ(Address::TYPE_MAC, myAddr.GetType());
...
zSocketTest_MacAddressCompare(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# zSocketTest_MacAddressCompare()");
  ZLOG_DEBUG("#############################################################");

  // Create new socket address and validate
  MacAddress myAddr1;
  TEST_EQ(Address::TYPE_MAC, myAddr1.GetType());
  TEST_EQ(std::string("00:00:00:00:00:00"), myAddr1.GetAddress());

  // Create second socket address and validate
  MacAddress myAddr2;
  TEST_EQ(Address::TYPE_MAC, myAddr2.GetType());
