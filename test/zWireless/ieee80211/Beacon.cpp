...
const uint8_t beacon_frame_wpa2[] =
{
	...
	// Country
	0x07, 0x0a, 0x44, 0x45, 0x20, 0x24, 0x08,
	0x14, 0x64, 0x0b, 0x1a, 0x00,
	//RSN
	0x30, 0x14, 0x01,
	0x00, 0x00, 0x0f, 0xac, 0x04, 0x01, 0x00, 0x00,
	0x0f, 0xac, 0x04, 0x01, 0x00, 0x00, 0x0f, 0xac,
...
const size_t beacon_frame_len_wpa2 = sizeof(beacon_frame_wpa2);
...
Ieee80211Test_BeaconAssembleWpa2(void* arg_)
{
  ...
  uint8_t frm_beacon[] =
  {
      ...
      0x00, 0x0f, 0xac, 0x02,
      0x2D, 0x1A, 0x34, 0x12,
      0x56, 0x78, 0x79, 0x7A, 0x7B, 0x7C, 0x7D, 0x7E, 
      0x7F, 0x80, 0x81, 0x83, 0x82, 0x84, 0x00, 0x00, 
      0x00, 0x86, 0x85, 0x8A, 0x89, 0x88, 0x87, 0x8B,
      // WMM/WME
      0xDD, 0x18, 0x00, 0x50, 0xF2, 0x02, 0x01, 0x01,
      0x80, 0x00, 0x03, 0xA4, 0x00, 0x00, 0x27, 0xA4, 
      0x00, 0x00, 0x42, 0x43, 0x5E, 0x00, 0x62, 0x32,
...
Ieee80211Test_BeaconDisassembleWpa2(void* arg_)
{
  ...
  // Make copy of raw beacon frame
  zSocket::Buffer sb;
  sb.Write(beacon_frame_wpa2, beacon_frame_len_wpa2);
  sb.Push(beacon_frame_len_wpa2); // reset data ptr to start of buffer
...
////  TEST_IS_ZERO(frame.PowerCaps().size());
  ...
  // Verify
  struct tim_tag tim_st= frame.Tim();
  ...
  TEST_EQ(0x60, frame.Rates()[6]); /* 48 */
  TEST_EQ(0x6c, frame.Rates()[7]); /* 54 */
  TEST_EQ(1, frame.Dsss.Length());
  TEST_EQ(4, frame.Tim.Length());
  TEST_EQ(1, frame.Tim.Count());
  TEST_EQ(10, frame.Country.Length());
