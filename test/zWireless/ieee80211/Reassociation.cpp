...
const uint8_t reass_req_frame[] =
{
    ...
    0x0a, 0x00, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, // interval, Current AP MAC
	0x00, 0x04, 0x47, 0x57, 0x30, 0x32, /* ....GW02 */
...
const size_t reass_req_frame_len = sizeof(reass_req_frame);

const uint8_t reass_resp_frame[] =
{
	0x30, 0x00,
...
const size_t reass_resp_frame_len = sizeof(reass_resp_frame);
...
Ieee80211Test_ReassociationRequestGetSet(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# Ieee80211Test_ReassociationRequestGetSet()");
  ZLOG_DEBUG("#############################################################");
  ...
  TEST_TRUE(frame.CurrentApMac("00:11:22:33:44:55"));
  ...
  TEST_EQ(std::string("00:11:22:33:44:55"), frame.CurrentApMac());
...
Ieee80211Test_ReassociationRequestAssemble(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# Ieee80211Test_ReassociationRequestAssemble()");
  ZLOG_DEBUG("#############################################################");
  ...
  uint8_t frm_rassreq[] =
  {
      0x20, 0x00, 0x34, 0x12, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
      0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x34, 0x12,
      0x34, 0x12, 0x64, 0x00, 0x00, 0x11, 0x22, 0x33,
	  0x44, 0x55, 0x00, 0x08, 0x54, 0x65, 0x73, 0x74,
	  0x53, 0x53, 0x49, 0x44, 0x01, 0x04, 0x01, 0x02,
	  0x04, 0x08, 0xdd, 0x04, 0x00, 0x50, 0xf2, 0x02
  };
  ...
  TEST_EQ(sb.Size(), sizeof(frm_rassreq));

  uint8_t* p = sb.Head();
  for (int i = 0; i < sizeof(frm_rassreq); i++)
  {
    TEST_EQ_MSG((int)frm_rassreq[i], *p++, zLog::IntStr(i));
  }
...
Ieee80211Test_ReassociationRequestDisassemble(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# Ieee80211Test_ReassociationRequestDisassemble()");
  ZLOG_DEBUG("#############################################################");

  // Make copy of raw association request frame
  zSocket::Buffer sb;
  sb.Write(reass_req_frame, reass_req_frame_len);
  sb.Push(reass_req_frame_len); // reset data ptr to start of buffer
...
Ieee80211Test_ReassociationResponseGetSet(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# Ieee80211Test_ReassociationResponseGetSet()");
  ZLOG_DEBUG("#############################################################");
  ...
  TEST_IS_ZERO(frame.ReassociationIdentifier());
  TEST_IS_ZERO(frame.Rates().size());
  ...
  TEST_TRUE(frame.ReassociationIdentifier(1));
  TEST_TRUE(frame.Rates(1));
  TEST_TRUE(frame.Rates(2));
  TEST_TRUE(frame.Rates(4));
  TEST_TRUE(frame.Rates(8));
  ...
  TEST_EQ(1, frame.ReassociationIdentifier());
  TEST_EQ(4, frame.Rates().size());
  TEST_EQ(1, frame.Rates()[0]);
  TEST_EQ(2, frame.Rates()[1]);
  TEST_EQ(4, frame.Rates()[2]);
...
Ieee80211Test_ReassociationResponseAssemble(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# Ieee80211Test_ReassociationResponseAssemble()");
  ZLOG_DEBUG("#############################################################");

  size_t len = 0;
  uint8_t frm_buf[64] = { 0 };
  uint8_t frm_rassresp[] =
  {
      0x30, 0x00, 0x34, 0x12, 0x01, 0x02, 0x03, 0x04,
  ...
  TEST_EQ(sb.Size(), sizeof(frm_rassresp));

  uint8_t* p = sb.Head();
  for (int i = 0; i < sizeof(frm_rassresp); i++)
  {
    TEST_EQ_MSG((int)frm_rassresp[i], *p++, zLog::IntStr(i));
  }
...
Ieee80211Test_ReassociationResponseDisassemble(void* arg_)
{
  ...
  ZLOG_DEBUG("#############################################################");
  ZLOG_DEBUG("# Ieee80211Test_ReassociationResponseDisassemble()");
  ZLOG_DEBUG("#############################################################");

  // Make copy of raw ReassociationResponse frame
  zSocket::Buffer sb;
  sb.Write(reass_resp_frame, reass_resp_frame_len);
  sb.Push(reass_resp_frame_len); // reset data ptr to start of buffer
  ...
  TEST_EQ(0x0431, frame.Capabilities());
  TEST_EQ(0, frame.Status());
  TEST_EQ(0xc001, frame.ReassociationIdentifier());
  TEST_EQ(8, frame.Rates().size());
  TEST_EQ(0x82, frame.Rates()[0]); /* 1(B) */
  TEST_EQ(0x84, frame.Rates()[1]); /* 2(B) */
  TEST_EQ(0x8b, frame.Rates()[2]); /* 5.5(B) */
